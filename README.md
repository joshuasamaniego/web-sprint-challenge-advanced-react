...
- In order to build stateful class components, you need to wrap an axios call or fetch request inside of componentDidMount(), as it acts like useEffect() in functional components, and whenever updating state you use this.setState(), instead of the useState React hook.

2. Describe the different phases of the component lifecycle.

- Start with Mounting Phase, with constructor function initializing the app's state, the app renders and then moves to the commit phase where finally componentDidMount() gets called. Then it goes into an updating phase, where a change in state causes a rerender and then componentDidUpdate() gets called. Finally, after all this the Unmounting phase happens once the specific component is finished via componentWillUnmount()
...
- [ y ] Run `npm install` to download dependencies for the server.
- [ y ] Run the server using `npm start`.
- [ y ] Open a new browser tab and navigate to `http://localhost:3333/plants` - if you see a JSON object with plants data, then you know the server is running 👍

**Starting the React app**

- [ y ] In a separate terminal cd into the `client` folder and run `npm install` to download dependencies.
- [ y ] Still inside the `client` folder run `npm run start` to run the client application.
- [ y ] Your browser should open up the project as normal
...
- [ y ] In the `PlantList` class component, fetch data from the server you now have running - the data can be fetched from `http://localhost:3333/plants`
- [ y ] Set the data to a state property called `this.state.plants`
- [ y ] The render function is already built and styled. Once the data is on the state, you will see the list of plants, and you will have the functionality to add a plant to the cart
...
- [ y ] Build out the tests listed in `CheckoutForm.test.js`. You will need to make sure they are testing what the test title implies they are testing
- [ not yet ] Make sure the tests are passing, and make sure you can cause the tests to fail purposefully, so that you know the tests are truly working
